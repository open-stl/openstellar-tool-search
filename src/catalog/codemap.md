...
`src/catalog/` serves as the centralized catalog management, schema normalization, and multi-tier hybrid search subsystem for OpenStellar Tool Search. It is responsible for:
...
- **Location**: `vault.ts`
- **Details**: `ToolVault` acts as the primary external boundary and façade for the entire catalog subsystem. It encapsulates internal components (`ToolStore` and `HybridSearchEngine`), coordinating state invalidation via event subscriptions (`this.store.onChanged(() => this.engine.notifyChanged())`), and exposes a clean, unified API for tool registration, lifecycle readiness polling, regex pattern matching, and hybrid search execution.
...
- **Details**: Decouples catalog mutations from search index freshness. When tools are added, updated, or removed in `ToolStore`, `notifyChanged()` triggers registered listeners in `HybridSearchEngine`. The search engine increments its `semanticGeneration` counter and marks `scorerStale = true` and `semanticStale = true`, deferring index rebuilding until query time (lazy evaluation).
...
### 7. Multi-Tier Retrieval & Reciprocal Rank Fusion Pipeline (`HybridSearchEngine`)
...
+-----------------------------------------------------------------------------+
   ...
   ToolStore.add()  ---> [Guard against [deferred] overwriting full descriptions]
           |
           v (fires onChanged)
   HybridSearchEngine.notifyChanged() ---> [scorerStale=true, semanticStale=true, generation++]
```
...
+-----------------------------------------------------------------------------+
   Client Query: ToolVault.query(text, limit, timeoutMs)
           |
           v
   HybridSearchEngine.query(text, limit, timeoutMs)
           |
   +-------+------------------------------------------+
   |                                                  |
   v                                                  v
...
1. **Catalog Ingestion & Invalidation**:
   ...
   - `ToolStore.notifyChanged()` notifies `HybridSearchEngine.notifyChanged()`, bumping `semanticGeneration`.

2. **Lexical Scoring Pipeline (`queryBM25`)**:
   - `HybridSearchEngine.buildScorer()` checks `scorerStale`; if true, instantiates a new `RankEngine<ToolMeta>(k1, b)`.
...
| :--- | :--- | :--- |
| `ToolVault` | `vault.ts` | Main façade orchestrating catalog storage and hybrid search. |
| `ToolStore` | `tool-store.ts` | In-memory store, alias resolver, parameter extractor, regex searcher. |
| `HybridSearchEngine` | `search-engine.ts` | Multi-tier coordinator combining BM25, semantic embeddings, and RRF. |

...
try {
  ...
  const { stdout } = await execFileAsync(
    ...
    [
      '--input-type=module',
      '-e',
      "import('@openstellar/tool-search').then(async (mod) => { if (!mod.default || typeof mod.default !== 'object') throw new Error('Default export is not an object'); if (typeof mod.default.server !== 'function') throw new Error('Default export missing server function'); if (typeof mod.ToolSearchPlugin !== 'function') throw new Error('ToolSearchPlugin export is not a function'); const ctx = { client: { tui: { showToast: async () => {} } } }; const resV2 = await mod.default.server(ctx, {}); if (!resV2 || typeof resV2 !== 'object') throw new Error('V2 plugin did not return hooks object'); const resV1 = await mod.ToolSearchPlugin(ctx, {}); if (!resV1 || typeof resV1 !== 'object') throw new Error('V1 plugin did not return hooks object'); console.log('plugin-smoke-ok'); })",
    ],
    { cwd: tempDir },
  );
